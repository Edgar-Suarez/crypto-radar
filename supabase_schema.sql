...
-- Habilitar Realtime en tablas que el frontend escucha
...
-- Row Level Security: lectura pública, escritura solo desde el backend (service key)
...
CREATE POLICY "public read" ON trending_coins    FOR SELECT USING (true);
CREATE POLICY "public read" ON trend_snapshots   FOR SELECT USING (true);
CREATE POLICY "public read" ON sentiment_history FOR SELECT USING (true);
CREATE POLICY "public read" ON market_mood       FOR SELECT USING (true);
CREATE POLICY "public read" ON alert_stream      FOR SELECT USING (true);
